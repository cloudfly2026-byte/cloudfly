...
export const contactService = {
  ...
  checkPhoneAvailability: async (phone: string): Promise<boolean> => {
    const url = `/api/v2/contacts/check-phone?phone=${phone}`;
    const response = await axiosInstance.get(url);
    return response.data;
  },

  checkEmailAvailability: async (email: string): Promise<boolean> => {
    const url = `/api/v2/contacts/check-email?email=${email}`;
    const response = await axiosInstance.get(url);
    return response.data;
  },

  checkDocumentAvailability: async (documentNumber: string, excludeId?: number): Promise<boolean> => {
    const url = `/api/v2/contacts/check-document?documentNumber=${documentNumber}${excludeId ? `&excludeId=${excludeId}` : ''}`;
    const response = await axiosInstance.get(url);
    return response.data;
  },
  
  searchContacts: async (query: string): Promise<Contact[]> => {
    const url = `/api/v2/contacts/search?q=${query}`;
    const response = await axiosInstance.get(url);
    return response.data;
  }
};
