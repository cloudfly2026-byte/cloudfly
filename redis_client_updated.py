...
class AsyncRedisClient:
    ...
    async def check_tool_idempotency(self, key: str, ttl: int = 60) -> bool:
        """Returns True if already exists (locked), False if new (acquired)."""
        idem_key = f"tool_idem:{key}"
        try:
            result = await self._redis.set(idem_key, "1", ex=ttl, nx=True)
            return result is None
        except Exception as exc:
            logger.error(f"Redis tool idempotency check failed: {exc}")
            return False
