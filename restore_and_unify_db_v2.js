  ...
  conn.sftp((err, sftp) => {
    ...
    sftp.fastPut('c:/apps/cloudfly/db/cloud_master_inicial_final_mayo_2026.sql', '/tmp/cloud_master_inicial_final_mayo_2026.sql', (err1) => {
      ...
      sftp.fastPut('c:/apps/cloudfly/add_new_features_tables.sql', '/tmp/add_new_features_tables.sql', (err2) => {
        ...
        // 3. Stop backend-api container to release active connection pools and database locks!
        console.log('🛑 Stopping backend-api container to release database locks...');
        conn.exec('docker stop backend-api', (errStop, streamStop) => {
          if (errStop) throw errStop;
          streamStop.stderr.on('data', d => process.stderr.write(d));
          streamStop.on('close', () => {
            console.log('✅ backend-api stopped successfully.');
            
            // 4. Execute original backup to restore all tables, data, and DIAN/channels columns
            ...
            conn.exec(cmdRestore, (err3, stream3) => {
              ...
              stream3.on('close', (code3) => {
                ...
                // 5. Execute new features schema (workflows, tags, notifications)
                ...
                conn.exec(cmdAddFeatures, (err4, stream4) => {
                  ...
                  stream4.on('close', (code4) => {
                    ...
                    // 6. Restart backend-api container
                    console.log('🚀 Restarting backend-api container...');
                    conn.exec('docker start backend-api', (errStart, streamStart) => {
                      if (errStart) throw errStart;
                      streamStart.stderr.on('data', d => process.stderr.write(d));
                      streamStart.on('close', () => {
                        console.log('✅ backend-api restarted successfully.');
                        
                        // 7. Generate unified, complete database dump
                        ...
                        conn.exec(cmdDump, (err5, stream5) => {
                          ...
                          stream5.on('close', (code5) => {
                            ...
                            // 8. Download the clean backup file
                            sftp.fastGet('/tmp/cloud_master_clean_install.sql', 'c:/apps/cloudfly/db/cloud_master_clean_install.sql', (err6) => {
                              ...
                              // 9. Cleanup temp files on VPS host
