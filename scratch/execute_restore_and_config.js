    ...
    sftp.fastPut('c:/apps/cloudfly/db/cloud_master_inicial_final_mayo_2026.sql', '/tmp/cloud_master_inicial_final_mayo_2026.sql', (err1) => {
      ...
      sftp.fastPut('c:/apps/cloudfly/add_new_features_tables.sql', '/tmp/add_new_features_tables.sql', (err2) => {
        ...
        const cmd = `
          ...
          echo "🔧 [CONFIG] Fixing IP connections to internal docker hosts in .env..."
          sed -i 's/DB_HOST=109.205.182.94/DB_HOST=mysql/g' /apps/cloudfly/.env
          sed -i 's/SPRING_R2DBC_URL=r2dbc:mysql:\\/\\/109.205.182.94/SPRING_R2DBC_URL=r2dbc:mysql:\\/\\/mysql/g' /apps/cloudfly/.env
          echo "✅ .env file successfully corrected on VPS."
          ...
          echo "🚀 Restarting backend-api container with fresh configurations..."
