# Script de restauración de emergencia para Exim
FILE="/etc/exim4/exim4.conf.template"
cat << 'RESTORATION_EOF' > "$FILE"
######################################################################
#                                                                    #
...
acl_check_spammers:
  ...
  # Rate limiting: máximo 10 conexiones por hora por IP
  defer   message = Too many connections from $sender_host_address. Try again later.
          log_message = Rate limit exceeded: $sender_host_address
          ratelimit = 10 / 1h / per_conn / $sender_host_address

  # Rate limiting: máximo 5 intentos de autenticación por hora por IP
  defer   message = Too many authentication attempts from $sender_host_address. Try again later.
          log_message = Auth rate limit exceeded: $sender_host_address
          condition = ${if !eq{$smtp_command_argument}{}}
          ratelimit = 5 / 1h / per_cmd / $sender_host_address
...
acl_check_rcpt:
  # Rate limiting: máximo 20 destinatarios por hora para IPs no autenticadas
  defer   !authenticated = *
          message = Too many recipients from $sender_host_address. Try again later.
          log_message = RCPT rate limit exceeded: $sender_host_address
          ratelimit = 20 / 1h / per_rcpt / $sender_host_address

  # Rate limiting: máximo 5 mensajes por hora para IPs no autenticadas
  defer   !authenticated = *
          message = Too many messages from $sender_host_address. Try again later.
          log_message = Message rate limit exceeded: $sender_host_address
          ratelimit = 5 / 1h / per_mail / $sender_host_address
...
# Aplicar whitelist en white-blocks.conf
IP="109.205.182.94"
WFILE="/etc/exim4/white-blocks.conf"
if ! grep -q "$IP" "$WFILE"; then
    echo "$IP" >> "$WFILE"
fi
